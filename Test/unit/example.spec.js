...
//Teste do controle de usuários
  ...
  const ControleUsuarios = require('../../app/Service/ControleUsuarios')
  ...
  test('Teste de Integração - Criação do usuário', async ({assert})=>{
      ...
      assert.equal(controle.insert(usuario), true)
  })

  test('Teste de Integração - Atualização do usuário', async ({assert})=>{
      let controle = new ControleUsuarios()
      assert.equal(controle.update(usuario), true)
  })

  test('Teste de Integração - Remoção do usuário', async ({assert})=>{
      let controle = new ControleUsuarios()
      assert.equal(controle.delete(usuario), true)
  })

  test('Teste de Integração - Login', async ({assert})=>{
      let controle = new ControleUsuarios()
      assert.equal(controle.login(usuario), true)
  })
